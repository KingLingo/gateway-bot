  ...
  <AppLayout>
    <div class="mx-auto max-w-4xl space-y-6">
      ...
      <template v-else>
        ...
        <div v-if="tabs.length > 1 && paymentPhase === 'select' && !selectedPlan" class="flex space-x-1 rounded-[6px] bg-gray-100 p-1 dark:bg-dark-800">
        ...
        <template v-else>
          ...
          <template v-else-if="activeTab === 'subscription'">
            ...
            <!-- Plan list -->
            <template v-else>
              <div v-if="customerPlans.length === 0" class="card py-16 text-center">
                <Icon name="gift" size="xl" class="mx-auto mb-3 text-gray-300 dark:text-dark-600" />
                <p class="text-gray-500 dark:text-gray-400">{{ t('payment.noPlans') }}</p>
              </div>
              <div v-else :class="planGridClass">
                <SubscriptionPlanCard v-for="plan in customerPlans" :key="plan.id" :plan="plan" :active-subscriptions="activeSubscriptions" @select="selectPlan" />
              </div>
              ...
              <div v-if="activeSubscriptions.length > 0">
                ...
                <div class="space-y-2">
                  <div v-for="sub in activeSubscriptions" :key="sub.id"
                    class="flex items-center gap-3 rounded-[6px] border border-gray-100 bg-white px-3 py-2 dark:border-dark-700 dark:bg-dark-800">
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="showRenewalModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" @click.self="closeRenewalModal">
          <div class="relative w-full max-w-lg rounded-[6px] border border-gray-200 bg-white p-6 shadow-2xl dark:border-dark-700 dark:bg-dark-900">
            <!-- Close button -->
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="previewImage" class="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 backdrop-blur-sm" @click="previewImage = ''">
          <img :src="previewImage" alt="" class="max-h-[85vh] max-w-[90vw] rounded-[6px] object-contain shadow-2xl" />
        </div>
      </Transition>
    </Teleport>
  </AppLayout>
...
const customerPlans = computed(() =>
  checkout.value.plans.filter((plan) => CUSTOMER_VISIBLE_PLATFORMS.has(plan.group_platform || ''))
)
