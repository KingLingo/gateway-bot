      ...
      <template v-else>
        ...
        <div class="space-y-4">
          ...
          >
            ...
            <div class="bg-gray-900 dark:bg-dark-900 rounded-[6px] overflow-hidden">
              <!-- Code Header -->
...
export XAI_API_KEY="${apiKey}"`
  ...
  // Shape follows Grok Build user guide (~/.grok/docs + custom-models) and production-ready Gateway Bot setups.
  ...
  const configContent = `# Grok Build CLI → Gateway Bot Grok group (API key auth).
# Docs: ~/.grok/docs/user-guide/05-configuration.md + 11-custom-models.md
# Verify after save: grok inspect
#
# IMPORTANT: api_backend must be "responses" for Gateway Bot Grok (POST /v1/responses).
...
# Prefer API key when using a custom gateway (matches Gateway Bot).
...
description = "Grok 4.5 via Gateway Bot (Responses)"
...
# Imagine tools: model IDs go to Gateway Bot media endpoints (not the text [model.*] catalog).
...
function generateGrokCodexFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  const configContent = `# Codex CLI → Gateway Bot Grok group
...
# Grok/Gateway Bot path is HTTP/SSE; disable WS (Codex may otherwise try WebSocket first)
...
function generateOpenCodeConfig(platform: string, baseUrl: string, apiKey: string, pathLabel?: string): FileConfig {
  ...
  if (platform === 'gemini') {
    ...
    // Custom provider pointing at Gateway Bot OpenAI-compatible Responses/Chat endpoints.
    provider[platform].npm = '@ai-sdk/openai-compatible'
    provider[platform].name = 'Grok via Gateway Bot'
